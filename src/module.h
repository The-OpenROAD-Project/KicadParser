...
public:
    ...
    component(const int id = -1,
        ...
        : m_id(id), m_name(name), m_isFront(isFront) {}
    //dtor
    ~component() {}
...
public:
    ...
    int getComponentId() const { return (m_layer == 0 ? m_comp_id.first : m_comp_id.second); }
    int getId() const { return m_id; }
    void setAngle(double &angle) { m_angle = angle; }
    void setX(double &x) { m_x = x; }
    void setY(double &y) { m_y = y; }
    void setLayer(int &layer) { m_layer = layer; }
    void setComponentIds(std::pair <int,int> ids) {
        m_comp_id = ids;
    } 
