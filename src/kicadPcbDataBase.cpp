...
bool kicadPcbDataBase::parseKicadPcb()
{
  ...
  std::string fileExtension = m_fileName.substr(found + 1);
  if (fileExtension != "kicad_pcb")
  {
    std::cerr << "Input file should be in .kicad_pcb format. Current input file extension is : " << fileExtension << std::endl;
    return false;
  }

  std::ifstream arg_infile;
  arg_infile.open(m_fileName, std::ifstream::in);
  if (!arg_infile.is_open())
    return false;
